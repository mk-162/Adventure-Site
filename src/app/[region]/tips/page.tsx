    ...
    <div className="min-h-screen pt-4 lg:pt-8 pb-12">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex flex-wrap items-center gap-2 text-xs lg:text-sm text-gray-500 mb-6">
          ...
          <Link href={`/${regionSlug}`} className="hover:text-primary">{region.name}</Link>
          <ChevronRight className="w-3 h-3" />
          <span className="text-primary font-medium">Tips</span>
        </nav>

        <header className="mb-8">
          <h1 className="text-3xl lg:text-4xl font-black text-primary mb-3">
          ...
          <div className="space-y-4 mb-10">
            {answers.map((item) => (
              <Link
                ...
                className="group flex items-start gap-4 p-5 bg-white rounded-xl border border-gray-200 hover:border-primary/30 hover:shadow-md transition-all"
              >
                <div className="shrink-0 mt-0.5">
                  <Lightbulb className="w-5 h-5 text-accent-hover" />
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-bold text-primary group-hover:text-accent-hover transition-colors mb-1">
                ...
                <ArrowRight className="w-5 h-5 text-gray-400 group-hover:text-accent-hover shrink-0 transition-colors" />
              </Link>
            ))}
          </div>
        ) : (
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            {otherRegions.map((r) => (
              <Link
                key={r.slug}
                href={`/${r.slug}`}
                className="flex items-center gap-2 px-4 py-2 bg-gray-50 rounded-full border border-gray-200 hover:border-primary/30 hover:shadow-sm transition-all text-sm font-medium text-primary"
              >
