...
async function ActivityComboPage({ regionSlug, activitySlug }: { regionSlug: string; activitySlug: string }) {
  ...
  if (activitiesData.length === 0 && !comboData) {
    ...
    return (
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 lg:py-8">
        <Breadcrumbs
          items={[
            { name: region.name, href: `/${region.slug}` },
            { name: activityType.name },
          ]}
          className="mb-4 lg:mb-6"
        />

        <div className="bg-gradient-to-br from-primary/5 to-accent-strong/5 rounded-2xl p-8 lg:p-12 text-center border border-slate-200 mb-10">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-accent-strong/10 mb-6">
            <Compass className="w-8 h-8 text-accent-strong" />
          </div>
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {otherRegions.map((r) => (
                <Link
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-white rounded-full border border-slate-200 hover:border-primary/30 hover:shadow-md transition-all text-sm font-medium text-primary"
                >
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {otherActivityTypes.map((type) => (
                <Link
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-white rounded-full border border-slate-200 hover:border-accent-strong/30 hover:shadow-md transition-all text-sm font-medium text-primary"
                >
                  {type.name}
                </Link>
              ))}
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <ButtonLink href={`/${region.slug}`} variant="secondary" size="lg">
              Explore {region.name}
  ...
  // The `activities` table has no per-row image, so every card on this combo
  // (all the same activity type) shares the type's real hero photo rather than
  // a fabricated or mismatched one.
  const cardImage =
    activityType.heroImage || `/images/activities/${activitySlug}-hero.jpg`;
  ...
  return (
    <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 lg:py-8">
      ...
      {/* Activity-driven UI (stats, listing) only when bookable
          ...
          No filter bar / sort control here — launch scope is ≤5 items per
          combo, so a filter UI would be a fake affordance with nothing to do. */}
      {activitiesData.length > 0 && (
      <>
      {/* Stats Row */}
      <div className="grid grid-cols-3 gap-3 mb-6 lg:mb-8">
        <StatTile label="Experiences" value={activitiesData.length} />
        <StatTile label="Operators" value={uniqueOperators.size} />
        <StatTile label="From" value={displayMinPrice ? `£${displayMinPrice}+` : "N/A"} />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4 lg:gap-6 mb-8">
        {activitiesData.slice(0, 3).map((item) => (
          <UniversalCard
            ...
            image={cardImage}
            imageAlt={item.activity.name}
            title={item.activity.name}
            href={`/activities/${item.activity.slug}`}
            pill={item.activity.difficulty ? <DifficultyBadge level={item.activity.difficulty} /> : undefined}
            meta={item.operator?.name}
            rating={item.operator?.googleRating ? parseFloat(item.operator.googleRating) : null}
            reviewCount={item.operator?.reviewCount ?? null}
            priceFrom={item.activity.priceFrom ? parseFloat(item.activity.priceFrom) : null}
          />
        ))}
        ...
        {featuredOperator && (
          <div className="col-span-1 sm:col-span-2 lg:col-span-2 w-full rounded-2xl bg-primary text-white overflow-hidden relative my-4 lg:my-6 min-h-[250px] flex items-center">
            ...
            <div className="relative z-20 p-6 lg:p-8 flex flex-col items-start gap-3 lg:gap-4 max-w-lg">
              <span className="bg-accent-strong text-white text-xs font-bold px-2 py-1 rounded uppercase tracking-wider">
                Featured Operator
              </span>
              <h2 className="text-xl lg:text-2xl font-bold leading-tight">{featuredOperator.name}</h2>
              <p className="text-white/80 text-sm lg:text-base">
                {featuredOperator.tagline || "Experience the best adventures with our top-rated partners."}
              </p>
              <ButtonLink href={`/directory/${featuredOperator.slug}`} variant="outline" size="md">
        ...
        <div className="overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0 sm:overflow-visible no-scrollbar">
          <div className="flex sm:grid sm:grid-cols-3 lg:grid-cols-4 gap-4 pb-2 sm:pb-0">
            {otherActivityTypes.map((type) => (
              ...
              >
                <div className="w-full aspect-[4/3] rounded-lg bg-slate-200 relative overflow-hidden mb-2">
                  <div className="absolute inset-0 bg-primary/10 flex items-center justify-center">
                    <span className="text-primary font-bold text-xs">{type.name}</span>
                  </div>
                </div>
                <p className="font-bold text-sm leading-tight text-primary group-hover:text-accent-strong transition-colors">{type.name}</p>
                <p className="text-xs text-slate-500 mt-0.5">Explore</p>
              </Link>
            ))}
            ...
            <Link href={`/${region.slug}`} className="flex-shrink-0 w-[140px] sm:w-auto group hidden lg:block">
              ...
              <p className="font-bold text-sm leading-tight text-primary group-hover:text-accent-strong transition-colors mt-2">
                Explore {region.name}
              </p>
              <p className="text-xs text-slate-500 mt-0.5">All adventures</p>
            </Link>
          </div>
        </div>
      </div>
