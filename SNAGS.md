...
1) Add labels/ARIA for directory filters (A11y quick fix)
...
1) Sync directory filters with the URL (shareable)
...
1) Add LocalBusiness schema for operator pages
...
1) Bind search input to query state (UX clarity)
...
+ [ ] Hero: Stats data (78 Adventures, 12 Regions, 54 Itineraries) disappears behind Search Box (z-index/layout issue).
+ [ ] Nav: "Pick Your Adventure" icons link to `/activities/type/[slug]` (e.g., /activities/type/mountain-biking). Change to root `/[slug]` (e.g., /mountain-biking).
+ [ ] SEO: Audit/Remove `/activities/type/*` pages if redundant (redirect to root `/activity` pages).
...
- [ ] Content: "48 hours of adrenaline" itinerary missing image on Itineraries page.
...
+ [ ] UI: Trip Cost is shown twice. Remove the top one, keep the detailed breakdown at the bottom.
...
+ [ ] Content: Add "Generic Cards" for filler activities:
  ...
  + "Local Walk"
  + "Shopping in town"
+ [ ] Mobile/Responsive: Trip Cost is invisible on mobile -> Move to bottom sticky bar or a dedicated tab.
+ [ ] Bug: Map button not functioning on Itinerary pages.
...
+ [ ] Filters: Add Type and Season filters (e.g., "Autumn 26", "Spring 27").
+ [ ] UI: Remove "Use your location" / Distance fields from Event Filter (irrelevant for travel).
+ [ ] Scope: Expand calendar to include "Kids Events" (not just hardcore adventure).
+ [ ] Feature: "Add an Event" button (prominent CTA for organizers).
+ [ ] Feature: Special Event Submission Form (requires proof/website link).
+ [ ] Design: Enhanced/Premium Event styling (Bigger box, Image, Description) for paying advertisers.
...
+ [ ] UI: Directory Search -> Popular location/activity buttons link to empty results. Search logic needs improvement.
...
+ [ ] Feature: Map locations (everywhere) need a "Directions" link (Google Logo) opening external map.

+ [ ] Feature: Clicking location icon/text should open a Modal Map with a pin (not just a text label).
...
+ [ ] Content: Video Unavailable? Check/Audit all YouTube links (e.g., "Some videos are available?").
...
+ [ ] Mobile/Layout: Activity page header data (Best Time, Price, Difficulty, etc.) is cropped/cut off on small screens.
+ [ ] Bug: Markdown syntax visible in text on Mountain Biking page (e.g., "it's *the* mountain").

+ [ ] Table/List: MTB Table Improvements:
  + [ ] Data: Verify ratings (are they real?).
  + [ ] Content: Add Image column/thumbnail to row.
  + [ ] Taxonomy: Add tags (Downhill, Singletrack, etc.).
  + [ ] Interaction: Row arrow must link to Activity Page.
  + [ ] Interaction: Clicking name/location icon → Open Map Modal with pin.
...
+ [x] Nav: Add menu for other options (Email, Channel Sponsorship, etc.) — needs content/business decision on ad products
  [ ] Remove this banner from the top of the advertise page. "Enhanced listings from £9.99/mo — TripAdvisor charges £65+. See plans" 
