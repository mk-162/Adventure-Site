# content-ops-073-operator-beddgelert-bikes

## Objective
Improve content item operator-beddgelert-bikes: Beddgelert Bikes.
Channel: commercial. Content type: operator. Route/slug: /directory/beddgelert-bikes.
Commercial tier: unclassified_operator.
Known blocker: Operator "Beddgelert Bikes" missing: no description, no cover image, no logo, no email, no Google rating | Operator "Beddgelert Bikes" missing: no description, no cover image, no logo, no email, no Google rating.
...
- Content item: operator-beddgelert-bikes
- Channel: commercial
- Content type: operator
- Route/slug: /directory/beddgelert-bikes
- Current status: qa_needed
- Priority: 34
- Task type: commercial_review
- Recommended skill: directory-premium-lure-model
- Required output path: data/research/content-ops/operator-beddgelert-bikes.json
